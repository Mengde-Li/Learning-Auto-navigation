...
def see_image():
    ...
    save_path=os.path.join(Abs_Path,#yiting_images_0717_paper") #在当前所在文件夹下创建新文件夹
...
def generate_mask():
    save_path=os.path.join(Abs_Path,"save_images")
...
def convert_depth_to_npy():
    ...
    if True:
        index=10
        print(os.path.join(Abs_Path,"save_images/depth_{}.png".format(index)))
        depth_image=cv.imread(os.path.join(Abs_Path,"save_images/depth_{}.png".format(index)),cv.IMREAD_UNCHANGED)
        ...
        np.save(os.path.join(Abs_Path,"save_images/depth_{}.npy").format(index),depth_image)
